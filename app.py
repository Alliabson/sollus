...
def get_status(row):
    ...
    # Usa a coluna 'dataVencimentoReal' (que agora contém o Nominal tratado)
    dt_venc = row['dataVencimentoReal'] 
    ...
    try:
        # 3. Prepara as datas para comparação (normalizar remove a hora)
        today = pd.to_datetime(date.today()).normalize()
        # Normaliza o Timestamp já tratado
...
with tab_receber:
    ...
    if df_receber_raw is None or df_receber_raw.empty:
        ...
        try:
            ...
            with col2_cr:
                # Usa a coluna Vencimento (date object) para o date_input
            ...
            # --- Aplicação dos Filtros (REGRAS DO BI) ---
            
            # Converte os inputs do filtro (date) para Timestamp (datetime64) para comparar
            ...
            # 1. Filtro de Data: Exclui linhas com data NaT para replicar o filtro de PERÍODO ESPECÍFICO.
            kpi_df = df_receber[
                (df_receber['dataVencimentoReal'].notna()) & # <-- EXCLUI NaT's / datas vazias
            ...
            # Calcula KPIs a partir do df_receber_filtered (Data E Status)
            ...
            # KPI "Recebido no Mês" (Este MÊS) - Ignora todos os filtros de data/status
            today = date.today()
            ...
            # Formata a data de vencimento (Vencimento é a coluna 'date' object)
